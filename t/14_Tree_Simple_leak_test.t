...
# affect the parent's relationships
{
    ...
    is($tree1->getChild(0), $tree2, '... our other relations are still intact');    
...
# can be reaped, but all else is fine
{
    ...
    my $removed = $tree1->removeChild($tree2);

    memory_cycle_ok($tree1, '... calling removeChild on the middle tree leaves the root with no relations');   
    
    is($removed->getChild(0), $tree3, '... our relations with the middle tree and lowest branch are still intact');    
